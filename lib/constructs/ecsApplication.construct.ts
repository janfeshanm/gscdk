...
export class EcsApplicationConstruct extends Construct{
    constructor(scope: Construct, id: string,_props:IEcsApplicationConstruct) {
        ...
        const deployStage = this.attachDeployAction(pipeline.pipeline,pipeline.output,ecr.repositoryUri, _props);

    }

    attachDeployAction(pipeline:Pipeline,buildOutput:Artifact,repositoryUri:string,_props:IEcsApplicationConstruct){

        const ecs = this.createEcs(_props.ecsConfig,_props.account,_props.region,repositoryUri);
    ...
    createBuildPipeline(_props:IPipelineConfig,account:string,region:string,repo:IRepository)
    {
    ...
    private createEcrImage(_props:ICreateEcrImage){
        const repository = Repository.fromRepositoryName(this, 'ExistingRepositoryByName', _props.name);
        return repository;
    }
